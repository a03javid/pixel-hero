...
/**
 * Help scene — displays game instructions in a retro pixel-art panel.
 *
 * Shows mission, controls, items, enemies, hazards and objective
 * using the same full-screen background as MenuScene.
 *
 * @class HelpScene
 */
class HelpScene {
  ...
  _drawTitle(ctx) {
    ...
    ctx.fillText("HOW TO PLAY", cx, 95);
  ...
  _drawSection(ctx, cx, startY, titleH, panelW, title, lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      this._drawBodyLine(ctx, panelX + 8, y, panelW - 16, lines[i]);
      y += BODY_LINE_HEIGHT;
    }
  ...
  /**
   * Draw a single body text line. Two-column layout when line contains "  ".
   * @param {CanvasRenderingContext2D} ctx
   * @param {number} x
   * @param {number} y
   * @param {number} maxW
   * @param {string} line
   */
  _drawBodyLine(ctx, x, y, maxW, line) {
    const parts = line.split("  ");
    if (parts.length === 2) {
      ctx.fillStyle = BODY_TEXT_COLOR;
      ctx.fillText(parts[0], x, y);
      ctx.fillStyle = SECONDARY_TEXT_COLOR;
      ctx.fillText(parts[1], x + maxW / 2, y);
    } else {
      ctx.fillStyle = BODY_TEXT_COLOR;
